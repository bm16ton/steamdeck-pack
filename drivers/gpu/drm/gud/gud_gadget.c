...
static int gud_gadget_check_buffer(struct gud_gadget *gdg, struct drm_client_buffer *buffer,
				    struct drm_display_mode *mode, u32 format)
{
	...
	return fb->format->format == format && fb->width == mode->hdisplay && fb->height == mode->vdisplay;
	return true;
}
					  ...
					  const struct gud_state_req *req, size_t size)
{
	...
	if (!gud_gadget_check_buffer(gdg, gdg->buffer, &mode, format)) {
		...
		vaddr = drm_client_buffer_vmap(buffer, map);
		if (IS_ERR(vaddr)) {
					     ...
					     struct drm_rect *rect)
{
	...
	dst = drm_client_buffer_vmap(buffer, map);
